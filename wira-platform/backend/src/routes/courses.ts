...
router.get('/:id', cacheMiddleware(1800), async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params

  try {
    const row = await get<Course>('SELECT * FROM courses WHERE id = ?', [id])
...
router.get('/:id/modules', cacheMiddleware(3600), async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params

  // Check cache first
  try {
    const cachedModules = await cacheService.get<CourseModule[]>(`course:${id}:modules`)
    if (cachedModules) {
      res.json({
        success: true,
    ...
    // Cache the result
    await cacheService.set(`course:${id}:modules`, courseModules, 3600)
...
router.get('/:id/quiz', cacheMiddleware(3600), async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params

  // Check cache first
  try {
    const cachedQuiz = await cacheService.get<QuizQuestion[]>(`course:${id}:quiz`)
    if (cachedQuiz) {
      res.json({
        success: true,
        quiz: cachedQuiz,
...
router.post('/:id/invalidate-cache', async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params
