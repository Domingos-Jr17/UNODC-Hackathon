...
class CourseModel {
  static async findAll(): Promise<Course[]> {
    const query = 'SELECT * FROM courses WHERE is_active = 1';
    return await all<Course>(query);
  }

  static async findById(id: string): Promise<Course | null> {
    const query = 'SELECT * FROM courses WHERE id = ?';
    return await get<Course>(query, [id]);
  }

  static async findModulesByCourseId(courseId: string): Promise<CourseModule[]> {
    // This is a simplified implementation - in a real application,
    // modules would likely be stored in a separate table
  ...
  static async findUnique(where: { id: string }): Promise<Course | null> {
    return await this.findById(where.id);
  }

  static async findMany(): Promise<Course[]> {
    return await this.findAll();
  }

  static async create(data: Omit<Course, 'id' | 'created_at' | 'updated_at'>): Promise<Course> {
