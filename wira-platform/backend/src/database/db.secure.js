...
function insertSampleData() {
  ...
  ngos.forEach(ngo => {
    db.run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `, [ngo.id, ngo.name, ngo.contact_person, ngo.phone, ngo.email, ngo.address]);
  });
  ...
  users.forEach(user => {
    try {
      ...
      logger.error('Error inserting sample user', { error: error.message, anonymousCode: user.anonymous_code });
    }
  });
