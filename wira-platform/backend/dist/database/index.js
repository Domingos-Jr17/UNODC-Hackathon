...
exports.insertSecureUser = insertSecureUser;
...
const DB_PATH = process.env.DATABASE_PATH || path_1.default.join(__dirname, '../../data/wira.db');
...
function insertSecureUser(userData) {
    return new Promise((resolve, reject) => {
        try {
            const encryptedData = encryption_1.default.encryptUserData(userData);
            db.run(`
...
function logAudit(userCode, action, tableName, recordId, oldValues, newValues, req) {
    const auditData = {
        user_code: userCode,
        action,
...
function insertSampleData() {
    ...
    users.forEach(user => {
        try {
            const encryptedData = encryption_1.default.encryptUserData(user);
            db.run(`
